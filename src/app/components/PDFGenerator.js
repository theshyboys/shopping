  ...
  useEffect(() => {
    const generatePDF = () => {
      try {
        ...
        doc.text(`Subtotal: THB${subtotal.toFixed(2)}`, 150, finalY, {
          align: "right",
        });
        doc.text(`Tax (7%): THB${tax.toFixed(2)}`, 150, finalY + 7, {
          align: "right",
        });
        doc.text(`Total: THB${total.toFixed(2)}`, 150, finalY + 14, {
          align: "right",
        });

        // Add thank you message
        doc.setFontSize(11);
        doc.text("Thank you for your purchase!", 105, finalY + 30, {
          align: "center",
        });
